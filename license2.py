...
# value_int = int(input(Fore.GREEN+"Welcome to the WARP+ Key Generator\nEnter the desired number of keys: "+Fore.CYAN))
check = False
while not check:
  ...
  while a < value_int and not check:
      ...
      try:
          ...
          with httpx.Client(
              ...
              print(Fore.GREEN+f"Account Type: {account_type}")
              print(Fore.GREEN+f"Data volume : {referral_count} GB")
              # {license}
              print(Fore.GREEN+f"License: {license}")
  
              # Writing a key to a file
  #            with open('data.txt', 'r') as file:
  #              lines = file.readlines()
  #            lines.insert(1, license + '\n')
  #
  #            with open('data.txt', 'w') as file:
  #              file.writelines(lines)
              import os
              os.system(f"echo '{license}'  >  data.txt")
